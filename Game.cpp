...
bool Game::is_not_tried(Input_Coordinates ic) {
    ...
    if (!present) {
        ...
        cout << "Coordinates X = " << ic.x << " Y =  " << ic.y
             << "are already attacked, try new coordinate:\n";
        fout << "Coordinates X = " << ic.x << " Y =  " << ic.y
             << "are already attacked, try new coordinate:\n";
    }
    for (itr = mymap.begin(); itr != mymap.end(); itr++) {
...
std::string read_data(tcp::socket& sock) {
    asio::streambuf buf;

    auto n = read_until(sock, buf, "\a");
    auto b = buffers_begin(buf.data());
    return std::string(b, b + n);
}

void write_data(std::stringstream& ss, tcp::socket& sock) {
...
void Game::play_sockets() {
    ...
    tcp::endpoint ep(asio::ip::address_v4::any(), 54000);
    // Step 3. Instantiating and opening an acceptor socket.
    tcp::acceptor acceptor(m_io_context, ep.protocol());
    ...
    // all steps for creating socket using asio are done.
...
void Game::play_game_s(tcp::socket&& sock) {
    Input_Coordinates ic;
    while (is_winner()) {
        std::stringstream ss;
        ...
        ss << "(Player 1 turn) ";
        write_data(ss, sock);
        // sock.wait(tcp::socket::wait_write);
